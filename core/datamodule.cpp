...
DataModule *DataModule::dm(QObject* parent)
{
    if (!_dm){
        _dm = new DataModule(parent);
    }
    return _dm;
}

QString DataModule::appPath()
{
    return AppSettings::applicationPath();
}

void DataModule::loadProjectData(int id)
{      
...
bool DataModule::saveProjectData(bool verbose)
{
...
void DataModule::checkForUnsavedProject(bool showDialog)
{
    ...
    if (!showDialog || QSmartDialog::confirmationDialog(SConfirmSaveModifiedProject)) {
        backupLocalProject();
        int errorRow = DataModule::dm()->checkStatementLengthExceeded();
        if (errorRow >= 0) {
...
void DataModule::saveLastStatement()
{
...
void DataModule::loadLastStatement()
{
    if (projectId > 0){
        QString section = SETTING_PROJECT_GROUP + QString::number(projectId);
...
int DataModule::nextId(QString sequenceName)
{
...
bool DataModule::execSql(const QString &sql)
{
    LOG << "SQL:" << sql;
...
bool DataModule::execSqlScript(QString script)
{
    if (!startTransaction()) {
        qWarning() << "Transaction begin fails";
        return false;
    }
    QStringList queries = TextProcessor::splitSqlScript(script);
...
bool DataModule::importProject(QString importPath)
{    
    ...
    QString text = TextProcessor::fileToString(importPath);
    ...
    foreach(QString rawLine, textLines){
        QStringList statementList
            = TextProcessor::splitStringBySize(rawLine, STATEMENT_SPLIT_SIZE, headerDelimiter);
...
bool DataModule::importFromXml(QString importPath)
{
    //Грузим пустой проект, чтобы очистить модели
    loadProjectData(0);
...
bool DataModule::deleteProject(int row)
{
...
bool DataModule::exportProject()
{
    ...
    for(int i=0; i<mStatements->rowCount(); i++){
        ...
        if (!multifileDelimiter.isEmpty() && statementText.trimmed() == multifileDelimiter){
            ...
            resStatement.clear();
            speakerAttrs.clear();
            TextProcessor::stringToFile(SExportFilePattern.arg(exportText), appPath() + PATH_DELIMITER + projectDir + PATH_DELIMITER + projectTitle + "_" + StrUtils::intToStr(fileIndex++, digitsCount) + ".xml");
            exportText.clear();
        }
        else {
    ...
    if (multifileDelimiter.isEmpty()){
        TextProcessor::stringToFile(SExportFilePattern.arg(exportText), appPath() + PATH_DELIMITER + projectDir + PATH_DELIMITER + projectTitle + ".xml");
    }
    else {
        result = TextProcessor::stringToFile(SExportFilePattern.arg(exportText), appPath() + PATH_DELIMITER + projectDir + PATH_DELIMITER + projectTitle + "_" + StrUtils::intToStr(fileIndex++, digitsCount) + ".xml");
    }

    return result;
}

bool DataModule::exportSpeakerList(QString outDir)
{
    ...
    return TextProcessor::stringToFile(exportList.join(SNewLine), outDir.append("/Speakers.txt"));
}

bool DataModule::exportSqlTableModel(LSqlTableModel* model, QString outPath)
{
    QStringList exportList;
    QSqlRecord rec;
    QString headerLine;
    ...
    return TextProcessor::stringToFile(exportList.join(SNewLine), outPath);
}

bool DataModule::editSetting(QString settingName, QString caption)
{
...
bool DataModule::startTransaction()
{
...
bool DataModule::commitTransaction()
{
...
bool DataModule::rollbackTransaction()
{
...
bool DataModule::loadModel(LSqlTableModel* model, QString table, QString sequence)
{
    bool result = false;
...
int DataModule::getLastRecordId(LSqlTableModel *model)
{
...
int DataModule::checkStatementLengthExceeded()
{
    int idx = -1;
...
void DataModule::setTableHeaders(QSqlTableModel *table, QStringList headers)
{
...
QString DataModule::processByReplacePatterns(QString statement, int patternType, bool logging)
{
    if (logging)
...
void DataModule::on_autosave_timeout()
{
    QString debugMsg;
...
void DataModule::initNewSpeaker(QSqlRecord &record)
{
...
void DataModule::onBeforeStatementInsert(QSqlRecord &rec)
{
