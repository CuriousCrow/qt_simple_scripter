...
#include "forms/twovaluesinputdialog.h"
...
{
  TwoValuesInputDialog twoValDialog(this);
...
void StatementWindow::on_btnAccentuator_clicked()
{
  ...
  if (statementText.contains("'") && !QSmartDialog::confirmationDialog(SConfirmAccentuatorRun))
    return;
  QString resText = TextProcessor::processByAccentuator(ui->memStatement->toPlainText());
...
void StatementWindow::loadCustomActions()
{
  ...
  QString customActStr =
      TextProcessor::fileToString(TextProcessor::configDir() + FILE_CUSTOM_ACTIONS);
...
void StatementWindow::createAddMenu()
{
  ...
  addMenu->addAction(SActionInsertStatement, QKeySequence(Qt::CTRL | Qt::Key_D), this, SLOT(on_addStatementMenuClicked()))->setProperty("Action", (int)Insert);
  addMenu->addAction(SActionSplitStatement, QKeySequence(Qt::CTRL | Qt::Key_S), this, SLOT(on_addStatementMenuClicked()))->setProperty("Action", (int)Split);
...
{  
  ...
  ui->cmbSpeaker->setModelColumn(DataModule::dm()->speakerTitleCol);
