...
void SpeakerWindow::setModel(LSqlTableModel *model)
{
    _model = model;
...
{
    QString roleTitle = DataModule::dm()->roleTitle;
    QString actorTitle = DataModule::dm()->actorTitle;
    QString professionTitle = DataModule::dm()->professionTitle;
    ...
    ui->listView->setModelColumn(DataModule::dm()->speakerTitleCol);
}
...
{
    if (DataModule::dm()->speakerTitleCol  == IDX_ROLE)
        DataModule::dm()->newSpeakerRole = NEW_SPEAKER;
    else if (DataModule::dm()->speakerTitleCol == IDX_ACTOR)
        DataModule::dm()->newSpeakerActor = NEW_SPEAKER;
