...
void AbstractRestTableModel::setServiceUrl(QUrl url, bool andRefresh)
{
  ...
  if (_iconColumnIdx < 0) {
    _iconColumnIdx = colIdxByName("name");
  }
  if (andRefresh)
    refresh();
}
...
void AbstractRestTableModel::registerCalcColumn(CalcColumn *column)
{
  _calcColumns.append(column);
}
...
bool AbstractRestTableModel::tryToSubmitAll()
{
  bool resOk = true;
  foreach (qlonglong key, _changes.keys()) {
    resOk = tryToSubmit(key);
    if (!resOk) {
      break;
    }
  }
...
int AbstractRestTableModel::colIdxByName(QString name) const
{
...
{
  int colIdx = colIdxByName(column);
...
{
  {
    ...
    //It's a trick for retrieving ID
...
LookupJsonColumn::LookupJsonColumn(QString name, AbstractRestTableModel *lookupModel, QString keyColumn, QString lookupColumn)
    : CalcColumn(name), _lookupModel(lookupModel), _keyColumn(keyColumn), _lookupColumn(lookupColumn)
{
}

QVariant LookupJsonColumn::data(int row, int role, const QJsonObject &jObj) const
{
  int keyVal = jObj.value(_keyColumn).toInt(-1);
  if (keyVal < 0) {
      return QVariant();
  }
  auto lookupIdx = _lookupModel->rowById(row);
  auto lookupValue = _lookupModel->cellData(lookupIdx, _lookupColumn, role);
  return lookupValue;
}

CalcColumn::CalcColumn(QString name) : _name(name)
{
}

void CalcColumn::setModel(AbstractRestTableModel *model)
{
  _model = model;
}

QVariant CalcColumn::modelData(int row, QString field, int role)
{
  if (field == _name) {
      qWarning() << "Infinite recursion in field: " << _name;
      return QVariant();
  }
  return _model->cellData(row, _name, role);
}
