...
{  
    ...
    if (role == Qt::EditRole &&
        ...
        LSqlRecord &rec = _recMap[_recIndex.at(index.row())];
        QMetaType fieldType = rec.field(index.column()).metaType();
...
bool LSqlTableModel::canConvert(const QVariant value, QMetaType type)
{
    if (value.metaType() == type)
        return true;

    if ((type.id() == QMetaType::Int || type.id() == QMetaType::LongLong) && value.typeId() == QMetaType::QString) {
...
QVariant LSqlTableModel::convert(const QVariant value, QMetaType type)
{
    if ((type.id() == QMetaType::Int || type.id() == QMetaType::LongLong) && value.typeId() == QMetaType::QString) {
        if (value.toString().isEmpty())
            return QVariant();
